...
export function App({ adapter = defaultAdapter, workspaceAdapter = defaultWorkspaceAdapter, renderSell, renderShop }: AppProps) {
  ...
  return <div className="unified-app" data-rail={collapsed ? "collapsed" : "expanded"}>
    ...
    <aside className="unified-rail" aria-label={copy.navigation}>
      ...
      <nav aria-label={copy.workspaces} className="rail-workspaces">{(["sell", "shop"] as const).map(item => <span key={item} className="workspace-link" data-current={workspace === item || undefined}>{item === "sell" ? <Radio size={20} /> : <ShoppingBag size={20} />}<span className="rail-label">{item === "sell" ? copy.sell : copy.shop}</span></span>)}</nav>
      <p className="rail-note rail-label">Connection · {mode === "preview" ? copy.browserPreview : sidecar?.state === "ready" ? copy.studioConnected : copy.studioOffline}</p>
