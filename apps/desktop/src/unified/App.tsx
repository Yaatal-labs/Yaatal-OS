...
export function App({ adapter = defaultAdapter, workspaceAdapter = defaultWorkspaceAdapter, renderSell, renderShop }: AppProps) {
  ...
  return <div className="unified-app" data-rail={collapsed ? "collapsed" : "expanded"}>
    ...
    <div className="unified-workspace">
      ...
      <Dialog open={accountOpen} onOpenChange={open => { if (!pending) { setAccountOpen(open); setPassword(""); setAccountError(""); } }}><DialogTrigger asChild><Button variant="outline" className="account-button" aria-label={session.authenticated ? session.merchant_name || copy.account : copy.signIn} disabled={initializing || mode === "checking" || mode === "blocked"}><UserRound size={18} /><span>{session.authenticated ? session.merchant_name || copy.account : copy.signIn}</span></Button></DialogTrigger><DialogContent onEscapeKeyDown={event => { if (pending) event.preventDefault(); }} onInteractOutside={event => { if (pending) event.preventDefault(); }}>
