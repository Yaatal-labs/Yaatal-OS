...
import { ArrowRight, ChevronLeft, ChevronRight, ClipboardList, Home, Moon, Package, Radio, Settings2, ShoppingBag, Sun, UserRound, UsersRound, WandSparkles } from "lucide-react";
...
import { AtelierWorkspace } from "./features/atelier/AtelierWorkspace";
import { resolveAtelierUrl } from "./features/atelier/atelierUrl";
...
export interface AppProps {
  ...
  /** Test seam; defaults to the embedded Yaatal Cloudflare OS. */
  renderAtelier?: () => ReactNode;
}
...
export function App({ adapter = defaultAdapter, workspaceAdapter = defaultWorkspaceAdapter, renderSell, renderShop, renderAtelier }: AppProps) {
  ...
  const [workspace, setWorkspace] = useState<Workspace>(() => { const saved = readPreference(WORKSPACE_KEY); return saved === "shop" || saved === "atelier" ? saved : "sell"; });
  // The Atelier mounts on first visit and then stays mounted, so switching back does not reload it.
  const [atelierVisited, setAtelierVisited] = useState(() => workspace === "atelier");
  useEffect(() => { if (workspace === "atelier") setAtelierVisited(true); }, [workspace]);
  ...
  return <div className="unified-app" data-rail={collapsed ? "collapsed" : "expanded"} data-workspace={workspace}>
    ...
    <aside className="unified-rail" aria-label={copy.navigation}>
      ...
      <nav aria-label={copy.workspaces} className="rail-workspaces">
        ...
        <Button variant="ghost" className="workspace-link" aria-label={railMessages[locale].openAtelier} aria-current={workspace === "atelier" ? "page" : undefined} data-current={workspace === "atelier" || undefined} onClick={() => navigate("atelier")}><WandSparkles size={20} /><span className="rail-label">{railMessages[locale].atelier}</span></Button>
    ...
    <div className="unified-workspace">
      <header className="unified-header"><span className="workspace-name">{workspace === "sell" ? copy.sellerWorkspace : workspace === "shop" ? copy.yourShop : copy.atelierWorkspace}</span><nav className="workspace-switch" aria-label={copy.workspaces}><Button variant="ghost" aria-current={workspace === "sell" ? "page" : undefined} onClick={() => navigate("sell")}>{copy.sell}</Button><Button variant="ghost" aria-current={workspace === "shop" ? "page" : undefined} onClick={() => navigate("shop")}>{copy.shop}</Button><Button variant="ghost" aria-current={workspace === "atelier" ? "page" : undefined} onClick={() => navigate("atelier")}>{copy.atelier}</Button></nav><div className="header-actions"><span className="connection" role="status" aria-live="polite"><i data-state={sidecar?.state} />{mode === "preview" ? copy.browserPreview : mode === "checking" || initializing ? copy.connecting : sidecar?.state === "ready" ? copy.studioConnected : copy.studioOffline}</span><Button variant="ghost" size="icon" className="shell-icon-control" aria-label={theme === "light" ? copy.useDark : copy.useLight} onClick={toggleTheme}>{theme === "light" ? <Moon size={18} /> : <Sun size={18} />}</Button><Button variant="ghost" size="icon" className="shell-icon-control locale-control" aria-label={locale === "en" ? copy.useFrench : copy.useEnglish} onClick={toggleLocale}>{locale === "en" ? copy.localeFrench : copy.localeEnglish}</Button><span className="header-divider" aria-hidden="true" />
      <Dialog open={accountOpen} onOpenChange={open => { if (!pending) { setAccountOpen(open); setPassword(""); setAccountError(""); } }}><DialogTrigger asChild><Button variant="outline" className="account-button shell-account-control" aria-label={session.authenticated ? session.merchant_name || copy.account : copy.signIn} disabled={initializing || mode === "checking" || mode === "blocked"}><UserRound size={18} /><span>{session.authenticated ? session.merchant_name || copy.account : copy.signIn}</span></Button></DialogTrigger><DialogContent onEscapeKeyDown={event => { if (pending) event.preventDefault(); }} onInteractOutside={event => { if (pending) event.preventDefault(); }}>
      ...
      <main id="workspace" ref={main} tabIndex={-1} className="unified-content">
        {mode === "checking" || initializing ? <p role="status">{copy.checking}</p> : mode === "blocked" ? <section className="empty-state"><p className="eyebrow">{copy.desktopConnection}</p><h1>{copy.cannotOpen}</h1><p role="alert">{runtimeError}</p><p>{copy.restartDesktop}</p></section> : <>
          ...
          {atelierVisited && <WorkspacePane active={workspace === "atelier"}>{renderAtelier?.() ?? <AtelierWorkspace url={atelierUrl} locale={locale} />}</WorkspacePane>}
          {context ? <>
