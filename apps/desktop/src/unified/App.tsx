...
export function App({ adapter = defaultAdapter, workspaceAdapter = defaultWorkspaceAdapter, renderSell, renderShop }: AppProps) {
  ...
  return <div className="unified-app" data-rail={collapsed ? "collapsed" : "expanded"}>
    ...
    <aside className="unified-rail" aria-label={copy.navigation}>
      ...
      <nav aria-label={copy.workspaces}>{(["sell", "shop"] as const).map(item => <Button key={item} variant="ghost" className="workspace-link" aria-label={item === "sell" ? copy.sell : copy.shop} aria-current={workspace === item ? "page" : undefined} onClick={() => navigate(item)}>{item === "sell" ? <Radio size={20} /> : <ShoppingBag size={20} />}<span className="rail-label">{item === "sell" ? copy.sell : copy.shop}</span></Button>)}</nav>
