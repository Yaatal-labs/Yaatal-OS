/**
 * AI Service - Smart Search Engine (PowerSync Version)
 * Voice search, NLP, visual search, and recommendations
 * Optimized for PowerSync/Supabase offline-first architecture
 */
...
export class VisualSearch {
  ...
  /**
   * Find similar products based on image (PowerSync version)
   * Uses PowerSync to query local SQLite database
   */
  static async findSimilarProducts(imageData: string): Promise<Product[]> {
    ...
    // For MVP, query local PowerSync database for recent products
    try {
      const query = `
      ...
      const products = await powerSyncService.executeQuery<Product>(query)
      return products
    } catch (error) {
...
// ============================================================================
// SMART RECOMMENDATIONS (PowerSync Version)
// ============================================================================

export class RecommendationEngine {
  /**
   * Get personalized recommendations based on user behavior
   * Uses PowerSync to query local search history and products
   */
  static async getPersonalizedRecommendations(
    ...
    try {
      // Get user's search history from local PowerSync DB
      const searchHistoryQuery = `
        ...
        LIMIT 50
      `
      const searchHistory = await powerSyncService.executeQuery<any>(searchHistoryQuery, [userId])
      ...
      searchHistory.forEach((search: any) => {
        ...
        if (search.keywords) {
          const keywords = Array.isArray(search.keywords) ? search.keywords : JSON.parse(search.keywords || '[]')
      ...
      const products = await powerSyncService.executeQuery<Product>(query, [...params, limit])
      return products
    } catch (error) {
   ...
   * Uses PowerSync for offline-first access
   */
  static async getTrendingProducts(limit: number = 10): Promise<Product[]> {
    try {
      ...
      const products = await powerSyncService.executeQuery<Product>(query, [limit])
      return products
    } catch (error) {
  ...
  static async getRelatedProducts(productId: string, limit: number = 6): Promise<Product[]> {
    try {
      ...
      const query = `
        SELECT * FROM products
        WHERE id != ? AND category = ? AND stock_quantity > 0
        ORDER BY upvotes DESC
        LIMIT ?
      `

      const relatedProducts = await powerSyncService.executeQuery<Product>(
        query,
        [productId, product.category, limit]
      )
...
// ============================================================================
// UNIFIED AI SEARCH SERVICE (HYBRID - PowerSync Version)
// ============================================================================

export class AISearchServicePowerSync {
  /**
   * Save search query to history for recommendations (PowerSync version)
   */
  static async saveSearchHistory(userId: string, intent: SearchIntent) {
    try {
      // Insert into search_history table via PowerSync
      const query = `
        INSERT INTO search_history (user_id, query, category, keywords, language, created_at)
        VALUES (?, ?, ?, ?, ?, datetime('now'))
      `

      await powerSyncService.executeQuery(query, [
        userId,
        intent.query,
  ...
  /**
   * Perform smart search with hybrid AI (PowerSync version)
   * Queries local PowerSync database for instant results
   */
  static async smartSearch(rawQuery: string, userId?: string): Promise<Product[]> {
    ...
    // Build SQL WHERE clause for PowerSync
    const conditions: string[] = ['stock_quantity > 0']
    const params: any[] = []
    ...
    if (intent.minPrice !== undefined) {
      conditions.push('(price >= ? OR discount_price >= ?)')
      params.push(intent.minPrice, intent.minPrice)
    }
    if (intent.maxPrice !== undefined) {
      conditions.push('(price <= ? OR discount_price <= ?)')
      params.push(intent.maxPrice, intent.maxPrice)
    }

    // Keyword search - search in title and description
    if (intent.keywords.length > 0) {
      const keywordConditions = intent.keywords.map(() => '(title LIKE ? OR description LIKE ?)')
    ...
    // Execute search on local PowerSync database
    const query = `
      SELECT * FROM products
      WHERE ${whereClause}
      ORDER BY upvotes DESC, created_at DESC
    ...
    try {
      const products = await powerSyncService.executeQuery<Product>(query, params)
      return products
    } catch (error) {
      console.error('PowerSync search error:', error)
      return []
    }
  }
...
export const AISearchService = AISearchServicePowerSync
